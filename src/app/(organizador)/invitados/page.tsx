...
export default function InvitadosPage() {
  ...
  const columns = [
    ...
    },
    {
      header: "Teléfono",
      accessor: (inv: Invitado) => (
        <Text size="2" color="slate" className="font-medium">
          {inv.telefono || "-"}
        </Text>
      ),
    },
    {
      header: "Mesa",
      accessor: (inv: Invitado) => {
        const mesa = useGuestStore.getState().mesas.find(m => m.id === inv.mesaId);
        return (
            <Badge variant="surface" color="slate" className="font-bold">
    ...
    <Flex direction="column" gap="6">
      <Flex justify="between" align="center">
        <Flex direction="column" gap="1">
          <Heading size="8" weight="bold" className="tracking-tight text-slate-900">
            Lista de Invitados
          </Heading>
          <Text size="2" color="slate" className="font-medium">
        ...
        <Dialog.Root open={isModalOpen} onOpenChange={setIsModalOpen}>
          <Dialog.Trigger>
            <Button size="3" color="violet" className="cursor-pointer font-bold px-5">
              <Plus size={16} /> Agregar Invitado
            </Button>
          </Dialog.Trigger>
          <Dialog.Content size="3" style={{ maxWidth: 440 }}>
            <Dialog.Title className="tracking-tight">Nuevo Invitado</Dialog.Title>
            ...
            <form onSubmit={handleSubmit}>
              <Flex direction="column" gap="4">
                <Box>
                  <Text as="label" size="2" weight="bold" className="text-slate-900 ml-1">
                    Nombre Completo
                  </Text>
                  <TextField.Root
                    mt="1"
                ...
                <Flex gap="3" justify="end" mt="2">
                  <Dialog.Close>
                    <Button variant="soft" color="slate" type="button" size="3">
                      Cancelar
                    </Button>
                  </Dialog.Close>
                  <Button type="submit" color="violet" size="3" className="font-bold px-6">
                    Guardar
                  </Button>
                </Flex>
              </Flex>
