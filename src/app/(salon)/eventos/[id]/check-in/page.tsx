...
import { ArrowLeft, CheckCircle2, User, Home, AlertTriangle, Scan, CameraOff, Info } from "lucide-react";
      ...
      try {
        ...
        if (mounted) {
          if (err?.includes?.("NotAllowedError") || err?.name === "NotAllowedError") {
            ...
            setError("El navegador bloquea la cámara en sitios no seguros (HTTP).");
          } else {
            setError("No se pudo acceder a la cámara. Verificá que no esté en uso.");
          }
          setIsScanning(false);
        }
      }
    ...
    <Box className="min-h-screen bg-slate-50 p-6 md:p-12">
      <Flex direction="column" gap="4" className="max-w-xl mx-auto">
        ...
        <Flex justify="between" align="center" mb="4">
          ...
          <Text size="1" weight="bold" className="uppercase tracking-widest text-slate-400">Control de Acceso</Text>
        ...
        {!isSecure && (
          <Callout.Root color="amber" variant="surface" size="1">
            ...
            <Callout.Text>
              Estás usando una conexión no segura (HTTP). La cámara <strong>no funcionará</strong> a menos que uses HTTPS o configures los flags de Chrome.
            </Callout.Text>
          </Callout.Root>
        )}

        <Box className="text-center space-y-2 py-4">
            <Heading size="8" weight="bold" className="tracking-tight">Puerta de Ingreso</Heading>
            <Text color="slate" className="font-medium">Escanea el código QR del invitado para confirmar asistencia.</Text>
        </Box>

        {/* Scanner View */}
        <Card size="3" className="overflow-hidden p-0 relative border-2 border-slate-200 shadow-xl">
          {isScanning && !error ? (
            <Box className="bg-black aspect-square flex items-center justify-center relative">
              ...
              <Flex className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/20 gap-2 items-center">
                ...
                <Text size="1" weight="bold" className="text-white uppercase tracking-wider">Buscando QR...</Text>
              </Flex>
            </Box>
          ) : (
            <Box className="p-8">
              {scannedGuest ? (
                <Flex direction="column" align="center" gap="6" className="text-center animate-in fade-in zoom-in duration-300">
                  <Box className="w-20 h-20 bg-green-50 text-green-600 rounded-full flex items-center justify-center shadow-inner">
                    <CheckCircle2 size={40} />
                  </Box>
                  
                  <Box className="space-y-1">
                    <Text size="1" weight="bold" className="uppercase tracking-widest text-green-600">Acceso Autorizado</Text>
                    <Heading size="7" weight="bold" className="tracking-tight">{scannedGuest.nombre}</Heading>
                  </Box>

                  <Card variant="surface" className="w-full bg-slate-50 border-slate-200">
                    <Flex direction="column" gap="4" p="2">
                        <Flex justify="between" align="center">
                            <Flex align="center" gap="2" className="text-slate-500">
                                <Home size={16} />
                                <Text size="2" weight="bold" className="uppercase tracking-wider">Ubicación</Text>
                            </Flex>
                            <Badge size="3" color="violet" variant="solid" className="font-bold px-4 py-1 rounded-md text-lg">
                        ...
                        <Flex justify="between" align="center">
                            <Flex align="center" gap="2" className="text-slate-500">
                                ...
                                <Text size="2" weight="bold" className="uppercase tracking-wider">Restricción</Text>
                            </Flex>
                ...
                <Flex direction="column" align="center" gap="6" className="text-center py-4">
                  <Box className="w-20 h-20 bg-red-50 text-red-600 rounded-full flex items-center justify-center">
                    <CameraOff size={40} />
                  </Box>
                  <Box className="space-y-1">
                    <Heading size="5" weight="bold" className="text-red-600">Error de Cámara</Heading>
                    <Text color="slate" className="font-medium">{error}</Text>
                  </Box>
                  <Button 
                    size="3" 
                    variant="soft" 
                  ...
                  {!isSecure && (
                    <Text size="1" color="amber" className="bg-amber-50 p-3 rounded-md border border-amber-200">
                      Para probar por IP local, entrá en <strong>chrome://flags/#unsafely-treat-insecure-origin-as-secure</strong> en Chrome y agregá tu IP.
                    </Text>
                  )}
                </Flex>
              ) : null}
        ...
        {/* Footer info */}
        <Flex justify="center" align="center" gap="2" className="text-slate-400 mt-4">
            <User size={14} />
            <Text size="1" weight="medium">Personal autorizado: Puerta Principal</Text>
        </Flex>
      </Flex>
    </Box>
  );
